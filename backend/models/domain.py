...
class Prescription(Base):
    ...
    cloudinary_public_id = Column(String(255), nullable=True) # Kept for backward compatibility if needed
...
class ClinicMedicine(Base):
    ...
    __tablename__ = "clinic_medicines"
...
class PrescriptionMedicine(Base):
    """Medicine instances attached to a specific prescription"""
    __tablename__ = "prescription_medicines"
    
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    prescription_id = Column(UUID(as_uuid=True), ForeignKey("prescriptions.id", ondelete="CASCADE"), nullable=False, index=True)
    clinic_medicine_id = Column(UUID(as_uuid=True), ForeignKey("clinic_medicines.id"), nullable=True)
    ...
    clinic_medicine = relationship("ClinicMedicine")
...
# Simple DB-based Token Blacklist
class TokenBlacklist(Base):
    __tablename__ = "token_blacklist"
    
    id = Column(Integer, primary_key=True, index=True)
    token = Column(String(500), unique=True, index=True, nullable=False)
    blacklisted_on = Column(DateTime(timezone=True), default=get_utc_now)

class AuditLog(Base):
    """Tracks critical file and entity events."""
    __tablename__ = "audit_logs"
    
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True, index=True)
    action = Column(String(100), nullable=False)  # e.g., UPLOAD_SIGNATURE, DOWNLOAD_PDF, REGENERATE_PDF
    entity_type = Column(String(100), nullable=False)  # e.g., PRESCRIPTION, DOCTOR
...
class ClinicSettings(Base):
    ...
    email = Column(String(255), default="info@elsanpublichealth.com")
    phone = Column(String(50), default="+91 94441 84977")
    website = Column(String(255), default="https://elsanpublichealth.com")
